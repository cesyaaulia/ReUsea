      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            backgroundColor: newStatus == 'Completed' ? AppTheme.ecoTeal : Colors.red,
  ...
  void _navigateToChat(Map<String, dynamic> item) async {
    ...
    try {
      ...
      String roomId = await _dbService.getOrCreateChatRoom(
        ...
        sellerId: sellerId,
        sellerName: item['sellerName'] ?? 'Penjual UNESA',
        ...
        Navigator.push(
          ...
          SlideFadeRightRoute(
            page: ChatDetailPage(
              roomId: roomId,
              peerId: sellerId,
              peerName: item['sellerName'] ?? 'Penjual UNESA',
              peerPhoto: item['sellerPhoto'] ?? '',
              product: dummyProd,
            ),
          ),
        );
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                FutureBuilder<DocumentSnapshot?>(
                                  future: item['sellerId'] != null && (item['sellerId'] as String).isNotEmpty
                                      ? FirebaseFirestore.instance.collection('users').doc(item['sellerId']).get()
                                      : Future<DocumentSnapshot?>.value(null),
                                  builder: (context, sellerSnap) {
                                    var userData = sellerSnap.data != null && sellerSnap.data!.exists
                                        ? sellerSnap.data!.data() as Map<String, dynamic>?
                                        : null;

                                    String sName = userData?['name'] ?? item['sellerName'] ?? 'Penjual UNESA';
                                    String sPhoto = userData?['photoUrl'] ?? item['sellerPhoto'] ?? '';
                                    double avgRating = (userData?['averageRating'] ?? 4.8).toDouble();
                                          ...
                                          Expanded(
                                            child: Column(
                                              ...
                                              children: [
                                                ...
                                                Row(
                                                  children: [
                                                    const Icon(Icons.star_rounded, color: AppTheme.sunYellow, size: 16),
                                                    const SizedBox(width: 4),
                                                    Text(
                                                      avgRating.toStringAsFixed(1),
                                                      style: GoogleFonts.lexend(color: AppTheme.secondaryBlue, fontSize: 12, fontWeight: FontWeight.bold),
                                                    ),
                                                    const SizedBox(width: 8),
                                                    Text(
                                                      "Joined: ${userData?['joinedAt'] ?? 'Januari 2026'}",
                                                      style: GoogleFonts.lexend(color: AppTheme.lightBlueGrey, fontSize: 11),
                                                    ),
                                                  ],
                                                ),
                                              ],
                                ...
                                if (_currentStatus == 'Waiting Confirmation')
                                  ...
                                  SizedBox(
                                    ...
                                    child: ElevatedButton.icon(
                                      ...
                                      label: Text(
                                        'Beri Ulasan Penjual',
                                        style: GoogleFonts.lexend(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15),
                                      ),
